...
std::string FormatBytes(std::size_t bytes) {
  ...
  out << std::fixed << std::setprecision(idx == 0 ? 0 : 1) << size
      << units[idx];
  return out.str();
}
...
int main() {
  ...
  while (!handler.IsFinished()) {
    ...
    std::cout << std::setw(8)
              ...
              << "/s  ";
    std::cout << "eta " << std::setw(8)
              << (smoothed_speed > 0 ? FormatTime(eta) : "---") << "  ";
    std::cout << std::flush;
  ...
  if (err.code != ErrorCode::Success) {
    std::cout << "DOWNLOAD WAS NOT SUCCESSFUL: " << err.detail << std::endl;
    return 1;
  }
